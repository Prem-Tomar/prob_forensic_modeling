...
| 1 — Image MVP | Deterministic decoder boundary, interpretable feature model, calibration, explanations, matched CIFAKE/SynthScars report, exact split digest, reusable hash-verified matched-family evaluator | Supply eligible local manifests and meet the approved ranking, calibration, and decision gates on at least three held-out families | Open |
...
| 3 — Provenance and confidence | Explicit provenance states, byte binding, late fusion, abstention, reliability bins, Wilson rate uncertainty, coverage/selective risk, safe local process adapter, signer trust policy, attributed hash-bound conformance runner | Run an approved C2PA verifier against licensed signed, tampered, revoked, unsupported, and absent-credential fixtures | Open |
| 4 — Robustness | Full deterministic CIFAKE codec/post-processing matrix, paired probability shifts, grouped uncertainty, diagnostic SynthScars transfer, frozen matched-control-source/device/category slices | Add three or more generator holdouts with complete governed slice metadata; meet the worst-slice gates without holdout tuning | Open |
| 5 — Temporal video | Deterministic sampling/decoding, DAVIS/Keling T2V+I2V/Sora source audit, scratch temporal model, reusable calibrated inference, bound frame calibration, portable attributed video manifests, matched-family comparison, timing | Supply matched real/generated clips and another training generator family; temporal AUROC must exceed the frozen frame baseline | Open |
| 6 — Reproducible release | Locked neural/build environments, hashed reports, byte-identical consecutive wheel builds, byte-identical smoke report from a fresh no-index wheel-only environment, exact-seed metric/tensor reproduction with semantic checkpoint and report hashes | Install the pinned neural environment from a local wheelhouse and repeat the neural reproduction in that clean offline environment | Open |
